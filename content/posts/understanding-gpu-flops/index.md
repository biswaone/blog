...
Matrices A, B, C, D of 4 X 4 dimensions.
...
To find the value of each element in the D matrix we have to do 4 FMA operation. We have 16 elements, in total we have to do 4 x 16 = 64 FMA operations.

In a modern Nvidia gpu like A100 or H100 there are several cores that can do these FMA operations.
...
That is 19.5 Tera-FLOPS per sec
...
Result: **4 clock cycles**, using 16 CUDA cores in parallel. Whereas using a Volta tensor core (generation before A100 tensor core) that can do 64 FMA per clock cycle the whole operation can be done in 1 clock cycle

A100 has 4 tensor core per SM, so a total of 108 x 4 = 432 cores with capability of 256 FPA/clock = 512 FLOPs/clock/core
